...
(defun xc/asm-main (code env tail)
  (cond ((null code) (cons nil tail))
        ((symbolp (car code))
         (let ((r (xc/asm-main (cdr code) env tail)))
           (cons (xc/asm-add-label (car code) (cdr r) (car r))
                 (cdr r))))
        (t (let ((opcode (caar code))
                 (args (cdar code))
                 (r (xc/asm-main (cdr code) env tail)))
             (setq tail (cdr r))
             (cons nil
